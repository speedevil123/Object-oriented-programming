...
namespace la8
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Columns[2].HeaderText = "Аудитория";

            dataGridView1.Columns[3].Width = 100;
            dataGridView1.Columns[3].HeaderText = "Время занятия";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < DataXML.Tables[0].Rows.Count; i++)
            {
                if (DataXML.Tables[0].Rows[i][0].ToString() == textBox5.Text)
                {
                    ...
                    string line = "Предмет: " + DataXML.Tables[0].Rows[i][1].ToString() + ", Аудитория: " +
                        DataXML.Tables[0].Rows[i][2].ToString() + ", Время: " + DataXML.Tables[0].Rows[i][3].ToString();
